...
(defparameter *vertex-data*
  (vector (make-test-vertex :pos #(-0.5 -0.5 0.0)
			    :color #(1.0 0.0 0.0)
			    :normal #(0.0 0.0 0.0))
...
(defun test ()
  (with-we-init (sys :w 800 :h 800 :x 0 :y 0 :title "test")
	...
	(withv-test-vertex (sys *vertex-data* vertex-buffer vertex-size)
	  (declare (ignore vertex-size))
	  (with-index-buffer (sys *index-data* index-buffer index-size)
	    (with-we-main-loop ()
	      (with-gcmd (cmd sys 0 0 800 800 #(1.0 1.0 1.0 1.0))
		(vk:cmd-bind-pipeline cmd :graphics pipeline)
		(vk:cmd-bind-vertex-buffers cmd 0 (list vertex-buffer) (list 0))
		(vk:cmd-bind-index-buffer cmd index-buffer 0 :uint32)
		...
		(vk:cmd-set-scissor cmd 0 (list (vk:make-rect-2d
						 ...
						 :extent (vk:make-extent-2d :width 800
									    :height 800))))
		(vk:cmd-draw-indexed cmd index-size 1 0 0 0)))))))))
